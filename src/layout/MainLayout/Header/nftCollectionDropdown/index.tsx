  ...
  return (
    <Grid justifyContent="space-between" container spacing={6}>
      ...
      <Grid item>
        ...
        <Menu id="simple-menu" anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => handleClose(null)}>
          {dataCollectionList?.collectionList.map((collection: any) => (
            <MenuItem key={collection.id} onClick={() => handleClose(collection)} style={{ color: 'black' }} disabled={!anchorEl}>
              {collection.title}
            </MenuItem>
          ))}
        </Menu>
